		...
		<section class="content">
			...
			<div class="row">
				<div class="container">
					...
					<div class="row">
						<form action="borrow_add" method="post" class="col s12">
							<div class="row">
								<div class="input-field col s6">
									<select id="idBook" name="idBook" class="browser-default">
										<option value="" disabled selected>-- Livres --</option>
										<c:forEach items="${books_available}" var="item">
											<option value="${item.getId()}">
												<c:out value="${item.getTitle()}" />, <i>
													<c:out value="${item.getAuthor()}" /></i></option>
										</c:forEach>
									</select>
								</div>
								<div class="input-field col s6">
									<select id="idMember" name="idMember" class="browser-default">
										<option value="" disabled selected>-- Membres --</option>
										<c:forEach items="${members_available}" var="item">
											<option value="${item.getId()}">
